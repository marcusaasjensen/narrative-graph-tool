...
namespace NarrativeGraphTool.Tests
{
    ...
    public class NarrativeRunnerTests
    {
        ...
        // ─── StartNarrative(fromNodeId) — save/resume ─────────────────────────────

        [Test]
        public void StartNarrative_WithNodeId_WithNoGraph_DoesNotSetRunning()
        {
            LogAssert.Expect(LogType.Error, "[NarrativeRunner] No NarrativeGraphData assigned.");
            _runner.StartNarrative("some_id");

            Assert.IsFalse(_runner.IsRunning);
        }

        [Test]
        public void StartNarrative_WithNodeId_InvalidId_LogsErrorAndDoesNotRun()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "Hi", "end"),
                GraphBuilder.End("end")));

            LogAssert.Expect(LogType.Error, "[NarrativeRunner] Cannot resume — node 'bad_id' not found in graph data.");
            _runner.StartNarrative("bad_id");

            Assert.IsFalse(_runner.IsRunning);
        }

        [Test]
        public void StartNarrative_WithNodeId_ResumesFromSpecifiedNode()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "First",  "line1"),
                GraphBuilder.Line("line1", "Second", "end"),
            ...
            NarrativeLineData fired = null;
            _runner.OnLine += d => fired = d;

            // Resume from the second node directly — first line should never fire
            _runner.StartNarrative("line1");

            Assert.AreEqual("Second", fired.text);
        }

        [Test]
        public void StartNarrative_WithNodeId_SkipsNodesBeforeTarget()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "First",  "line1"),
                GraphBuilder.Line("line1", "Second", "end"),
            ...
            _runner.OnLine += d => fired.Add(d.text);

            _runner.StartNarrative("line1");

            Assert.AreEqual(1, fired.Count);
            Assert.AreEqual("Second", fired[0]);
        }

        [Test]
        public void StartNarrative_WithNodeId_ResumesFromChoiceNode()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "Before choice", "choice0"),
                GraphBuilder.Choice("choice0",
                    GraphBuilder.Option("Yes", "end"),
                    GraphBuilder.Option("No",  "end")),
                GraphBuilder.End("end")));

            ChoiceNodeData choiceFired = null;
            _runner.OnChoice += d => choiceFired = d;

            _runner.StartNarrative("choice0");

            Assert.IsNotNull(choiceFired);
            Assert.AreEqual(2, choiceFired.options.Count);
        }

        [Test]
        public void StartNarrative_WithCurrentNodeId_SimulatesSaveAndRestore()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "First",  "line1"),
                GraphBuilder.Line("line1", "Second", "end"),
            ...
            // Run to first line and capture its node ID (simulate saving)
            _runner.StartNarrative();
            string savedId = _runner.CurrentNode.id;

            // Restore from saved ID
            NarrativeLineData restored = null;
            _runner.OnLine += d => restored = d;
            _runner.StartNarrative(savedId);

            Assert.AreEqual("First", restored.text);
        }

        [Test]
        public void StartNarrative_WithNodeId_SetsIsRunning_True()
        {
            _runner.SetGraphData(BuildGraph("line0",
                GraphBuilder.Line("line0", "Hi", "end"),
                GraphBuilder.End("end")));

            _runner.StartNarrative("line0");
